...
**Автор:** Maksim Kovalev · **Версия:** 1.3.0 · **Лицензия:** GPL-3.0
...
- **Порог компонент** — чек, флагающий тысячи компонент (sharp edges на all-soft хардсёрфе достигает 300k+), держит честный счётчик, но хранит ограниченный равномерный сэмпл для оверлея; Select для таких находок отключён.
...
- **Флаги координаторской станции** — через меню «...»: «Start in Coordinator Mode» открывает панель сразу в координаторском виде; «Coordinator Lock» прячет все фиксы в Coordinator Mode (куратор смотрит и пишет вердикт, чинит артист).
- **Empty Groups** — сцен-левел проверка трансформов без детей (мусор после пересборок/импортов); выделить и удалить вручную.
